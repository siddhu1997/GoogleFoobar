...
'''
UnionFind from: https://github.com/Dmdv/PADS/blob/master/UnionFind.py
Utilfrom: https://github.com/Dmdv/PADS/blob/master/Util.py
...
URL: https://www.ics.uci.edu/~eppstein/PADS/CardinalityMatching.py

Algorithm: Edmond's Blossom-Contraction Algorithm
...
#Our function from here onwards!

def infinitywar(x,y):
	result = (x + y)/gcd(x,y)
	return not (result & (result - 1)) == 0 and result > 0

def uGraph(banana_list):
	graph = {i: [] for i in range(len(banana_list))}

	for i,a in enumerate(banana_list):
		for j,b in enumerate(banana_list):
			if i != j and infinitywar(a,b):
				graph[i].append(j)
	return graph

def solution(banana_list):
	_graph_ = uGraph(banana_list)
	matches = matching(_graph_)
